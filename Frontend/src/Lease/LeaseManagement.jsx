...
const LeaseManagement = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/getleases")
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched Leases:", data); // Debugging API Response
        setLeases(data);
      })
    ...
    <div className="ml-64 p-6 bg-gray-100 min-h-screen">
      ...
      {drawerOpen && (
        <div className="fixed top-0 right-0 w-80 h-full bg-white shadow-lg p-4 overflow-y-auto">
          ...
          <button className="bg-red-600 text-white px-2 py-1 mt-2 rounded" onClick={() => setNotifications([])}>
            Clear All
          </button>
          <ul className="mt-4">
            {notifications.map((note, index) => (
              <li key={index} className="border-b py-2 text-sm">
                {note.message} <span className="text-gray-500 text-xs">({note.time})</span>
              </li>
            ))}
          </ul>
        </div>
